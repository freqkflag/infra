#!/usr/bin/env python3
"""
Preflight agent implementation.
"""
...
class PreflightAgent(BaseAgent):
    """Run the infra preflight checks via scripts/preflight.sh."""
    ...
    def build_arg_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            ...
            help="Infisical environment to use.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, parser: argparse.ArgumentParser, args: argparse.Namespace) -> int:
        ...
        self._log_event(f"preflight --env={env} -> {status}")
    ...
    def _log_event(self, message: str) -> None:
        ...
        entry = f"{timestamp} - preflight-agent - {message}"
...
def main(argv: Sequence[str] | None = None) -> int:
    agent = PreflightAgent(
        "preflight-agent",
        config={
            "description": "Run scripts/preflight.sh via Infisical.",
        },
    )
