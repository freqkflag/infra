...
**Domain(s):** grafana.freqkflag.co  
**URL(s):** https://grafana.freqkflag.co
...
- Tag runnable snippets with Runme metadata so Cursor can surface the dispatcher menu:
  ```markdown
  ```bash {"id":"monitoring-restart","runme":{"label":"Restart Monitoring Stack","tags":["runbook","monitoring"]}}
  cd /root/infra/monitoring
  docker compose restart
  ```
  ```
...
```
Use the Multi-Agent Orchestrator preset. Scope analysis to /root/infra/monitoring (Monitoring Stack) first, then capture upstream dependencies from compose.orchestrator.yml. Return strict JSON with prioritized run actions plus any restart commands needed for Monitoring Stack.
```

**Service Health Check**
```
Act as status_agent. Focus on /root/infra/monitoring and the monitoring containers. Confirm docker compose ps, health checks, and Traefik routing. Return strict JSON with health summary + restart recommendation.
```

**Targeted Operational Command**
```
Act as Deployment Runner. Execute the following commands for Monitoring Stack at /root/infra/monitoring:
cd /root/infra/monitoring
docker compose up -d
Verify the service at https://grafana.freqkflag.co and capture logs if unhealthy.
```
...
```bash
docker exec monitoring [health-check-command]
```
...
- Location: `/root/infra/monitoring/.env`
- Key variables:
  - `[VAR1]` - [description]
  - `[VAR2]` - [description]

### Docker Compose
- Location: `/root/infra/monitoring/docker-compose.yml`
...
- Application logs: `/root/infra/monitoring/data/logs/`
...
### Performance Issues
1. Check resource usage: `docker stats monitoring`
...
- [Service README](../monitoring/README.md)
