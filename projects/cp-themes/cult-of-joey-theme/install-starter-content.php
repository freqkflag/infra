<?php
/**
 * Starter Content Installation Script
 * 
 * This script can be accessed via browser or run via WP-CLI
 * Access via: http://yoursite.com/wp-content/themes/cult-of-joey-theme/install-starter-content.php
 */
...
$option_name = 'cult_of_joey_starter_content_installed';
if (get_option($option_name)) {
    echo '<h1>Starter content already installed!</h1>';
    echo '<p>To reinstall, first delete the option: <code>cult_of_joey_starter_content_installed</code></p>';
    exit;
}

echo '<h1>Setting up Cult of Joey Starter Content...</h1>';
echo '<pre>';
...
foreach ($moods as $mood) {
    if (!term_exists($mood, 'mood')) {
        $result = wp_insert_term(ucfirst($mood), 'mood', array('slug' => $mood));
...
foreach ($workshop_categories as $cat) {
    if (!term_exists($cat, 'workshop_category')) {
        $result = wp_insert_term($cat, 'workshop_category', array('slug' => strtolower($cat)));
        if (!is_wp_error($result)) {
...
foreach ($gallery_categories as $cat) {
    if (!term_exists($cat, 'gallery_category')) {
        $result = wp_insert_term($cat, 'gallery_category', array('slug' => strtolower($cat)));
        if (!is_wp_error($result)) {
            echo "  ✓ Created category: {$cat}\n";
        }
    } else {
...
echo "\nNext steps:\n";
echo "1. Activate the Cult of Joey theme (if not already active)\n";
echo "2. Set a page as your homepage (Settings > Reading)\n";
echo "3. Set a page for blog posts (Settings > Reading > Posts page)\n";
echo "4. Add featured images to posts, workshops, and gallery items\n";
echo "5. Customize content as needed\n";
echo "</pre>";
