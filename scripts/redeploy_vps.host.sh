#!/usr/bin/env bash
#
# Idempotent redeploy routine for the vps.host node.
# Wraps docker compose bundle(s) stored under nodes/vps.host/.
...
Any additional arguments are treated as docker compose service filters.
EOF
}
